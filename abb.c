...
TIPOCLAVE _clave_elem(TIPOELEMENTOABB *E) { return E->name; }
...
unsigned esAbbVacio(TABB A) { return A == NULL; }

TABB izqAbb(TABB A) { return A->izq; }

TABB derAbb(TABB A) { return A->der; }

void leerElementoAbb(TABB A, TIPOELEMENTOABB *E) { *E = A->info; }
...
TIPOELEMENTOABB
_suprimir_min(TABB *A) { // Se devuelve el elemento más a la izquierda
  TABB aux;
  TIPOELEMENTOABB ele;
  if (esAbbVacio(
          (*A)->izq)) { // Si izquierda vacía, se devuelve valor nodo actual A
    ele = (*A)->info;
    aux = *A;
    *A = (*A)->der;
    free(aux);
    ...
    return _suprimir_min(&(*A)->izq); // se vuelve a buscar mínimo rama
                                      // izquierda
  }
}
